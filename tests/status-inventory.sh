...
generated_dns_names() {
    # deployer 2.3+: names in /tmp/hosts/yggdrasil-*, zone from dns.conf
    load find_canonical_domain
    DNS_HOSTS_DIR="$TMP/hosts"; DNS_CONF="$TMP/dns.conf"; mkdir -p "$DNS_HOSTS_DIR"
    printf 'iface ygg0\nzone spb.home.arpa\nrouter router\n' > "$DNS_CONF"
    printf '%s\n' '203:170f:3ab2:166e:6d0d:af56:91ec:43fc router.spb.home.arpa' \
        '303:170f:3ab2:166e::10 zeonux.spb.home.arpa' > "$DNS_HOSTS_DIR/yggdrasil-ygg0"
    uci() { return 1; }
    find_canonical_domain Zeonux || fail 'generated name not found'
    eq '303:170f:3ab2:166e::10' "$CANONICAL_IPV6"
    eq 'zeonux.spb.home.arpa' "$DNS_ALIAS"
    # a name only in another zone does not match
    printf 'zone home.arpa\n' > "$DNS_CONF"
    if find_canonical_domain zeonux; then fail 'matched across zones'; fi
    # an operator record wins over the generated one
    printf 'zone spb.home.arpa\n' > "$DNS_CONF"
    uci() { case "$*" in
        '-q get dhcp.@domain[0]') return 0 ;;
        '-q get dhcp.@domain[0].name') echo 'zeonux.spb.home.arpa' ;;
        '-q get dhcp.@domain[0].ip') echo '303:170f:3ab2:166e::99' ;;
        *) return 1 ;; esac; }
    find_canonical_domain zeonux || fail 'operator record not found'
    eq '303:170f:3ab2:166e::99' "$CANONICAL_IPV6"
    unset -f uci
}
...
run 'names generated by the deployer are canonical in their zone' generated_dns_names
