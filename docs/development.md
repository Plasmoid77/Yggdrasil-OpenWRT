...
The deployer follows the newest published status release rather than a version
baked into the script, so shipping a status module does not require shipping a
deployer. It queries `releases/latest`, accepts the answer only as a
`status-vX.Y[.Z]` tag matching a strict pattern, and only then builds a download
URL — a release name can never steer the path. `--status-version vX.Y` selects a
specific release; `--status-pkg PATH` installs a local build and still requires
its single-entry `PATH.sha256`.

Every path verifies bytes before installing. A network install fetches the
`.sha256` published beside the archive in the same release; a checkout copy must
carry its own checksum file. Missing, ambiguous, mismatched or unfetchable
checksums refuse the optional status module, as do unavailable tools. Core
routing remains independent.

**Know what this does and does not prove.** The published checksum and the
archive come from the same release, so verification catches truncation and
corruption but not a compromised release; the earlier embedded pin additionally
covered substitution *after* a trusted deployer copy was obtained, at the cost
of a deployer change per release. That trade was made deliberately. Operators
who need the stronger property should build the module themselves and install it
with `--status-pkg`. Neither scheme authenticates a deployer script obtained
from an untrusted source. There is no automatic fallback to an older module.
...
This resolves the newest published release for real, downloads its archive and
published checksum, verifies them against each other, and compares against a
preserved `packages/` archive when one exists for that version. Host tests cover
synthetic installation and failure paths; they do not replace the real-router
matrix below. Before merging a downloader change, review the host/HTTP results
and the remaining hardware-validation boundary explicitly.
...
Publishing is the last step: the deployer picks the newest published release up
...
status distribution (v5.2) name different things. Do not renumber old history.
