...
| Retry down peers when an uplink comes up (2.5) | Yggdrasil doubles the pause between reconnection attempts up to 1h8m and cannot tell that the uplink is back: after an LTE modem replug the public peers returned 15 minutes after the uplink. Its defaults stay; `/etc/hotplug.d/iface/70-yggdrasil-peers` re-adds (`removepeer` + `addpeer` through the admin socket) the configured peers that are not up on every `ifup` of another interface, which restarts their attempts at once. Established and interface-bound peers are not touched |
