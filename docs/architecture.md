...
| `maxbackoff=1m` on every `--peer` (2.4) | Yggdrasil doubles the pause between reconnection attempts up to 1h8m by default, so peers lost in an uplink outage came back up to an hour after the uplink (15 minutes after an LTE modem replug on the SPb router). Its own per-peer option caps the pause at a minute; a URI with its own `maxbackoff` is kept as given |
